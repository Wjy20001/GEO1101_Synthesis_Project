...
def get_xyz_coordinates_base(user_image_name):
    ...
    matched_row = image_matching_df[
        image_matching_df["User image name (image you took)"]
        == user_image_name
    ]
    ...
    ground_truth_images = (
        matched_row.iloc[0, 3:8].dropna().tolist()
    )  # Extract relevant columns for Ground Truth images
    base_image_names = [
        img.split("_")[0] for img in ground_truth_images
    ]  # Ignore suffixes like _right, _front

    # Step 2: Match these base image names with the data in orbit_import_camera
    matched_xyz = orbit_import_df[
        orbit_import_df["Image"].str.contains("|".join(base_image_names))
    ][["Image", "X", "Y", "Z"]]
...
for i, dists in enumerate(distances):
    for j in range(
        1, len(dists)
    ):  # Start from the 1st nearest neighbor (2nd index)
        if (
            dists[j] < threshold
...
result = get_xyz_coordinates_base("validation_021.jpg")
...
for _, row in result.iterrows():
    plt.plot(
        row["X"], row["Y"], "ro", markersize=3
    )  # Red points for matched images
