...
def parse_args():
    ...
    parser.add_argument(
        "dataset_dir",
        type=str,
        help="Dataset folder containing the 'frames' folder with images.",
    )
    parser.add_argument(
        "--clusters",
        type=int,
        default=200,
        help="Number of clusters for KMeans (default: 200).",
    )
...
def main():
    ...
    if not os.path.exists(image_dir):
        print(
            f"'frames' folder not found in the dataset directory: {image_dir}"
        )
        return
    ...
    bow_vectors = [
        calculate_bow_vector(
            orb.detectAndCompute(cv2.imread(img, cv2.IMREAD_GRAYSCALE), None)[
                1
            ],
            kmeans,
        )
