...
if __name__ == "__main__":
    ...
    reference_data_file = os.path.join("data", "training", 'model.pkl')
    csv_path_coord = os.path.join("data", "csvs", "slam_camera_coordinates_merged_v2.csv")
    ...
    N_best_matches = 5 #default = 6
    min_DBSCAN_samples = 2 #default = 3


    diagnostics_csv_path = os.path.join("data", "diagnostics", f"diagnostics_{diagnostics_toggle}_N={N_best_matches}_cs={min_DBSCAN_samples}.csv")
    ...
    if diagnostics_toggle == 'single':
        ...
        filtered_df[['found_room', 'calculation_time']] = filtered_df['user_image_name'].apply(
            lambda x: get_room_name_and_time(x, rooms_json_path, N_best_matches, min_DBSCAN_samples)
        ).apply(pd.Series)
