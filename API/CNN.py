...
def apply_dbscan_and_find_center(all_coords, eps=0.5, min_samples=3):
    ...
    largest_cluster_center = tuple((float(largest_cluster_center[0]), float(largest_cluster_center[1])))
...
def match_query_images_and_get_center(query_image_paths, reference_data_file, csv_path, top_n_matches=6, min_DBSCAN_samples=3):
    ...
    for query_image_path in query_image_paths:
        print(f"Processing query image:\t\t{os.path.basename(query_image_path)}")
